...
Function ExecuteSql-Primary($sql)
{
	if ($global:UseZSSConnection) {
		...
		Invoke-Sqlcmd -ConnectionString $global:primarySQLConnectString
	}
}
...
Function Prepare-And-Filter-Table($primaryDatabase, $secondaryDatabase, $DBFile, $tName, $tWhereClause, $tSkippedColumns, $anyone = $false)
{
	...
	if ($tWhereClause)
	{
#		"Filtering table " + $tName + " Where " + $tWhereClause
		FilterTable $primaryDatabase $secondaryDatabase $DBFile $tName $tWhereClause $null $anyone
	}
}
...
Function Prepare-Table($db, $dbfile_name, $table_name, $skipped_columns = "")
{
    ...
    if (!$table.Prepared)
    {
        ...
        #            "  Warning: " + $warning
			...
			{
				$table.SkippedColumns.Add($skipColumn);
			}
            # prepare
